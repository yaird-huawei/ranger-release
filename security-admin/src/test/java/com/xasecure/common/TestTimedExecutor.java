...
public class TestTimedExecutor {
    ...
    public void test() throws InterruptedException {
		 ...
		 * assuming of course that the two submitted tasks would not finish before 3rd one is submitted.
		 */
        int poolSize = 2;
        int queueSize = 3;
        TimedExecutorConfigurator _configurator = new TimedExecutorConfigurator(poolSize, queueSize);
		 ...
		 * - 2 should succeed
		 * - 5 should timeout
		 *   - 2 in pool because they would never be able to acquire semaphore
		 *   - 3 in queue because they would never get scheduled to run since the above 2 would continue to hog the pool
		 *     threads.
		 * - 3 should get rejected becuase they can't get on the queue.
		 */
        Semaphore semaphore = new Semaphore(2);
		/*
		 * We need a latch to keep track of when the processing is done so we can check the results of the test
		 */
