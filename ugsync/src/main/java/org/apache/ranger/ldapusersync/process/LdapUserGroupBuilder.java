	...
	public void updateSink(UserGroupSink sink) throws Throwable {
		...
		try {
			...
			try {
					...
					while (userSearchResultEnum.hasMore()) {
						...
						if (groupSearchEnabled && groupUserMapSyncEnabled) {
							...
							try {
								...
								LOG.error("LDAPUserGroupBuilder.updateSink() compute memberships failed with exception: " + t);
							}
						}
				...
				LOG.error("LDAPUserGroupBuilder.updateSink() failed with exception: " + t);
				LOG.info("LDAPUserGroupBuilder.updateSink() user count: "
						+ counter);
			}
			...
			if (groupSearchEnabled && !groupUserMapSyncEnabled) {
				...
				try {
					...
					LOG.error("LDAPUserGroupBuilder.updateSink() failed during group search with exception: " + t);
				}
